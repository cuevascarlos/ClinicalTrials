...
3. For training models with `TrainingNER.py`:
    ...
    **Remark.** The only thing that has to be changed manually for reproducibility is the argument `eval_dataset` in Trainer during the training step (not in hyperparameters fine-tuning). To do Experiment 1 `eval_dataset=tokenized_datasets['valid']`, for Experiment 2 `eval_dataset=tokenized_datasets['test']`.
